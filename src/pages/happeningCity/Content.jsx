        ...
        <div className="flex justify-evenly items-center p-6 space-x-6  rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition duration-500">
          <div className="flex bg-gray-100 p-4 md:w-72 space-x-4 rounded-lg">
            ...
            <input
              ...
              onChange={(e)=>{setSearch(e.target.value)}} type="text" id="search"   
      ...
      {city.map((cityName) => (
        <p
          ...
          className={`m-5 text-white-400 hover:underline hover:cursor-pointer ${
            cityName.toLowerCase() === selectedLocation.toLowerCase() ? 'underline' : ''
          }`}
      ...
      events
        .filter((event) => {
          const locationMatches = event.location.some(
            ...
            selectedLocation && event.location.some((loc) => loc.city.toLowerCase() === selectedLocation.toLowerCase());

          return locationMatches && (selectedLocation ? selectedLocationMatches : true);
        })
        .map((event) => (
          <div
            key={event._id}
